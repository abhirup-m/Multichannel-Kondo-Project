...
\usepackage{braket,amsmath,amssymb,graphicx,float,hyperref,paralist,caption}
...
\hfill
\begin{minipage}{0.45\textwidth}
	\includegraphics[width=\textwidth]{rg_flow_pms.pdf}
	\captionof{figure}{Attractive finite \(J\) fixed point of poor man scaling RG equation}
\end{minipage}

\(J_1^*,J_2^*\) are two critical points of the equation where either the numerator or the denominator changes sign: \(J_1^* = \frac{32 D_0}{K}, J_2^* = 4D\). In the thermodynamic limit, \(J_1^* \gg J\), so the numerator is always positive, and the flow of \(J\) is determined by the denominator. For \(J_0 < J_2^*\), \(J\) is relevant and flows up towards \(J_2^*\). On the other hand, if \(J_0 > J_2^*\), then we have an irrelevant coupling which again flows down towards \(J_2^*\)
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\textwidth]{./rg_flow.pdf}
	\caption{Attractive RG flows towards the stable intermediate coupling fixed point at \(J_1^*\).}
\end{figure}
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.8\textwidth]{./K=2,rel_and_irr.pdf}
	\caption{Attractive RG flows towards the stable intermediate coupling fixed point at \(J_1^*\).}
\end{figure}
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.4\textwidth]{./Jfp_vs_D.pdf}
	\caption{Variation of the fixed point value \(J^*\) against increasing bandwidth \(D_0\). The single-channel curve (red) keeps rising while the two-channel curve (blue) saturates to a finite value.}
\end{figure}
