...
Defining \(\epsilon^h_m = \frac{1}{2}\left(Jm + h\right) \), the \(2\times 2\) blocks can be written as
\begin{equation}\begin{aligned}
	H_m = \begin{pmatrix} \epsilon^h_m & J/2 \\ J/2 & -\left( \epsilon^h_m + J/2 \right)   \end{pmatrix} 
\end{aligned}\end{equation}
The eigenvalues are 
\begin{equation}\begin{aligned}
	\lambda_{m, \pm}^{M, h} = \frac{1}{2}\left[-J/2 \pm \sqrt{5J^2/4 + 4\epsilon^h_m\left(\epsilon^h_m + J/2\right) }\right] = \frac{1}{2}\left[-J/2 \pm \sqrt{J^2 + 4\left(\epsilon^h_m + J/4\right)^2 }\right]= -J/4 \pm \sqrt{J^2/4 + \alpha^2}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	&=\sum_{M=M_\text{min}}^{M_\text{max}}\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}2e^{\beta J/4}\cosh \beta\sqrt{J^2/4 + \alpha^2} + 2e^{-\beta JM/2}\cosh \beta h/2\right]
\end{aligned}\end{equation}
where \(M_\text{max} = K/2\) for a \(K-\)channel Kondo model, and \(M_\text{min} = 0\)  if \(K\) is even, otherwise \(1/2\).
...
\end{aligned}\end{equation}
where the \(\prime\) indicates derivative with respect to \(h\). We will now calculate these derivatives. For that we w11ill need
\begin{gather}
...
\begin{equation}\begin{aligned}
	\frac{\:\mathrm{d}Z(h)}{\:\mathrm{d}h} &= \beta\sum_{M=M_\text{min}}^{M_\text{max}}\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}e^{\beta J/4}\frac{\alpha\sinh \beta\sqrt{J^2/4 + \alpha^2} }{\sqrt{J^2/4 + \alpha^2}} + e^{-\beta JM/2}\sinh \left(\beta h/2\right)\right] \\
	\frac{\:\mathrm{d}^2Z(h)}{\:\mathrm{d}h^2} &= \beta\sum_{M=M_\text{min}}^{M_\text{max}}\left[\frac{1}{2}\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}\frac{e^{\beta J/4}}{\sqrt{J^2/4 + \alpha^2}}\left(\sinh \beta\sqrt{J^2/4 + \alpha^2} + \beta\alpha^2 \frac{\cosh\beta\sqrt{J^2/4 + \alpha^2}}{\sqrt{J^2/4 + \alpha^2}} - \frac{\alpha^2 \sinh \beta\sqrt{J^2/4 + \alpha^2}}{J^2/4 + \alpha^2}\right)\right.\\
						   &\quad \quad\quad \quad\left.+ e^{-\beta JM/2}\frac{\beta}{2}\cosh \left(\beta h/2\right)\right] \\
\end{aligned}\end{equation}
We will now take the limit of \(h \to 0\). Note that \(\alpha(h\to 0)=\frac{J}{2}(m+1/2)\) and hence \((J^2/4 + \alpha^2)(h \to 0) = \frac{J^2}{4}\left[1 + (m+1/2)^2\right] \).
\begin{equation}\begin{aligned}
	\lim_{h \to 0}Z(h) &= \sum_{M=M_\text{min}}^{M_\text{max}}\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}2e^{\beta J/4}\cosh \beta \frac{J}{2}\sqrt{1 + (m+1/2)^2} + 2e^{-\beta JM/2}\right] \\
	\lim_{h \to 0}\frac{\:\mathrm{d}Z(h)}{\:\mathrm{d}h} &=  \beta\sum_{M=M_\text{min}}^{M_\text{max}}\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}e^{\beta J/4}(m+1/2)\frac{\sinh \beta\frac{J}{2}\sqrt{1 + (m+1/2)^2} }{\sqrt{1 + (m+1/2)^2}}\right] \\
	\lim_{h \to 0}\frac{\:\mathrm{d}^2Z(h)}{\:\mathrm{d}h^2} &= \beta\sum_{M=M_\text{min}}^{M_\text{max}}\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}\frac{e^{\beta J/4}}{J\sqrt{1 + (m+1/2)^2}}\left(\sinh \beta\frac{J}{2}\sqrt{1 + (m+1/2)^2} + \beta \frac{J}{2}(m+1/2)^2 \frac{\cosh\beta\frac{J}{2}\sqrt{1 + (m+1/2)^2}}{\sqrt{1 + (m+1/2)^2}} \right.\right.\\
						   &\quad \quad\quad \quad\left.\left. - \frac{(m+1/2)^2 \sinh \beta\frac{J}{2}\sqrt{1 + (m+1/2)^2}}{1 + (m+1/2)^2}\right)+ e^{-\beta JM/2}\frac{\beta}{2}\right]
\end{aligned}\end{equation}
...
For \(K=2\), we have \(M = 0,1\). For \(M=0\) there is no scattering and the only eigenvalue is 0. For \(M=1\), we have \(m=-1,0,1\). The terminal states are \(m=-1, S_d^z=-\frac{1}{2}\) and \(m=1, S_d^z=+\frac{1}{2}\). The scattering blocks are \(m=-\frac{1}{2}\) and \(m=0\). This means that \((m+1/2)^2 = 1/4\) for all relevant \(m\). Substituting all this in the expressions gives
\begin{equation}\begin{aligned}
	\lim_{h \to 0}Z(h) &= 2 + 2e^{-\beta J/2} + 2e^{\beta J/4}\sum_{m=-1,0}\cosh \beta \frac{J}{2}\sqrt{1 + (m+1/2)^2} = 2\left(1 + e^{-\beta J/2}\right) + 4e^{\beta J/4}\cosh \frac{\beta J\sqrt 5}{4}\\
	\lim_{h \to 0}\frac{\:\mathrm{d}Z(h)}{\:\mathrm{d}h} &= \beta\sum_{m=-1,0}e^{\beta J/4}(m+1/2)\frac{\sinh \beta\frac{J}{2}\sqrt{1 + (m+1/2)^2} }{\sqrt{1 + (m+1/2)^2}} = 0\\
	\lim_{h \to 0}\frac{\:\mathrm{d}^2Z(h)}{\:\mathrm{d}h^2} &= \frac{\beta^2}{2}\left(1 + e^{-\beta J/2}\right)+ \frac{\beta}{J}\frac{e^{\beta J/4}}{5}\left(\frac{16}{\sqrt 5}\sinh \beta\frac{J\sqrt 5}{4} + \beta J \cosh\beta\frac{J\sqrt 5}{4}\right)\\
	\chi &= \frac{\frac{\beta}{2}+ \frac{1}{J}\frac{e^{\beta J/4}}{5\left(1 + e^{-\beta J/2}\right)}\left(\frac{16}{\sqrt 5}\sinh \beta\frac{J\sqrt 5}{4} + \beta J \cosh\beta\frac{J\sqrt 5}{4}\right)}{2 + 4\frac{e^{\beta J/4}}{\left(1 + e^{-\beta J/2}\right)}\cosh \frac{\beta J\sqrt 5}{4}}
\end{aligned}\end{equation}

The low temperature behaviour in general is
\begin{equation}\begin{aligned}
	\chi(T \to 0) = \frac{\beta}{4}\frac{(K-1)^2}{4 + (K-1)^2}
\end{aligned}\end{equation}
