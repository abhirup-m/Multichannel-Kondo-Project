...
The eigenvalues are 
\begin{equation}\begin{aligned}
	\lambda_{m, \pm}^M(h) = \frac{1}{2}\left[-J \pm \sqrt{2J^2 + 4\epsilon^h_m\left(\epsilon^h_m + J\right) }\right] 
\end{aligned}\end{equation}
The eigenvalues of the terminal states are \(\pm\epsilon^h_{\pm M}\). The partition function is
\begin{equation}\begin{aligned}
	Z(h) = \sum_{M}\left[\sum_{m=-M+1}^{M-1}e^{-\beta \lambda_{m, \pm}^M(h)} + e^{-\beta \epsilon^h_{M}} + e^{\beta \epsilon^h_{- M}}\right] 
\end{aligned}\end{equation}
