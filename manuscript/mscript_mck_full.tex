...
\end{aligned}\end{equation}
\(H^i_{(j)}\) is that part of \(H_{(j)}\) that commutes with \(\hat n_j\) but does not commute with at least one \(\hat n_l\) for \(l < j\). The RG flow continues till a fixed point is reached at an energy \(D^*\). The Greens function in front of $c_j$ (in Eq.~\ref{eta_dag}) accounts for the dynamical cost of this rotation in terms of an operator for quantum fluctuations (\(\hat \omega\)), as well as the kinetic and self-energies \(\text{Tr}\left( H_{(j)}\hat n_j \right) \).
...
\end{align}
The Hamiltonian commutes with \(\vec S\), so it is already block-diagonal in terms of the eigenvalues \(M\) of \(S\). \(M\) takes values in the range \(\left[M_\text{min}, M_\text{max}\right]\), where \(M_\text{max} = K/2\) for a \(K-\)channel Kondo model, and \(M_\text{min} = 0\)  if \(K\) is even, otherwise \(\frac{1}{2}\). The diagonalisation of the Hamiltonian is shown in the Supplementary Materials~\cite{SM}. Defining \(\alpha = \frac{1}{2}\left({\mathcal{J}}m + h\right) + \frac{{\mathcal{J}}}{4}\) and \(x^M_m = M(M+1) - m(m+1)\), the partition function can be written as
\begin{eqnarray}
...
\begin{equation}\begin{aligned}
	\ket{J^z=\sigma \frac{K-1}{2}} = \frac{1}{\sqrt{1 + K}} \ket{\sigma}_d\otimes\ket{S^z=\sigma(\frac{K}{2}-1)} - \frac{\sqrt K}{\sqrt{1 + K}}\ket{\bar\sigma}_d\otimes\ket{S^z=\sigma\frac{K}{2}}~, ~ ~ \sigma=\pm 1~,
\end{aligned}\end{equation}
where \(\ket{\sigma}_d\) represents the up and down configurations of the impurity spin and \(\ket{S^z}\) represents the configuration of the bath spins. These states lead to large unquenched magnetisation at zero temperatures. These states lead to singular behaviour of the impurity magnetisation in vanishing field, \(m(h=0^\pm)\). In the presence of a field, the ground-state becomes unique: \(\ket{\psi}_\text{gs}(h) = \theta(h)\ket{J^z=\frac{K-1}{2}} + \theta(-h)\ket{J^z=-\frac{K-1}{2}}\), and the impurity magnetisation can be calculated from the state:
\begin{equation}\begin{aligned}
	m(h=0^\pm) = \theta(h)\bra{J^z=\frac{K-1}{2}} S_d^z \ket{J^z=\frac{K-1}{2}} + \theta(-h)\bra{J^z=-\frac{K-1}{2}}S_d^z\ket{J^z=-\frac{K-1}{2}} = \pm  \frac{1}{2} \frac{1-K}{1 + K}\label{imp-magnetisation}~.
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
When the phase shift \(\delta^{(l)}\) acquires a fractional value (in units of \(\pi\)), the impurity magnetisation also becomes fractional (in units of \(\frac{1}{2}\)). This shows that the unrenormalised phase shift value \(0 < \delta^{(l)} < \pi\) is at the heart of the quantum frustration present in the star graph (and therefore the MCK problem) that neither allows the impurity to be screened, nor allows it to decouple from the conduction channels. The decoupling only happens in the limit of extreme screening \((K \to \infty)\), and the impurity spin then becomes a local moment.
