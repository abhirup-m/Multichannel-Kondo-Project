...
\section{Derivation of low energy effective \(k-\)space Hamiltonian }
...
\section{Susceptibility from zero-mode fixed-point Hamiltonian}
...
The eigenvalues of the terminal states are \(\pm\epsilon^h_{\pm M}\). The partition function is
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	...
	\frac{\:\mathrm{d}^2Z(h)}{\:\mathrm{d}h^2} &= \beta\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[\frac{1}{2}\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}\frac{e^{\beta J/4}}{\sqrt{J^2x^M_m/4 + \alpha^2}}\left(\sinh \beta\sqrt{J^2x^M_m/4 + \alpha^2} + \beta\alpha^2 \frac{\cosh\beta\sqrt{J^2x^M_m/4 + \alpha^2}}{\sqrt{J^2x^M_m/4 + \alpha^2}} - \frac{\alpha^2 \sinh \beta\sqrt{J^2x^M_m/4 + \alpha^2}}{J^2x^M_m/4 + \alpha^2}\right)\right.\\
						   &\quad \quad\quad \quad\left.+ e^{-\beta JM/2}\frac{\beta}{2}\cosh \left(\beta h/2\right)\right] \\
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
For brevity, we define \(\theta_M = \beta J (M+1/2)/2\) and \(\Sigma_M = \sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}(m+1/2)^2\).
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	...
	\chi &\to \frac{\beta\Sigma_\text{max}}{2M_\text{max}\left(2M_\text{max}+1\right)^2} = \frac{\beta\Sigma_\text{max}}{K(K+1)^2}
\end{aligned}\end{equation}
...
\section{Bath contribution to Susceptibility from zero-mode fixed-point Hamiltonian}
...
\begin{equation}\begin{aligned}
	\frac{\:\mathrm{d}\phi_m^M}{\:\mathrm{d}h} &= -\frac{J(m+1/2)/2 + h(4m - 1)/2}{\phi_m^M} \rightarrow \lim_{h \to 0}\rightarrow -\frac{J(m+1/2)/2}{\phi^M}\\
	\lim_{h \to 0}\frac{\:\mathrm{d}^2\phi_m^M}{\:\mathrm{d}h^2} &= \frac{1/2 -2m}{\phi^M} - \frac{J^2}{4}\frac{(m + 1/2)^2}{\left(\phi^M\right)^3}\\
\end{aligned}\end{equation}
Substituting these give
\begin{equation}\begin{aligned}
	     Z(h \to 0) &=\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}2e^{\beta J/4}\cosh \beta\phi^M + 2e^{-\beta JM/2}\right]\\
	Z^\prime(h \to 0) &=-\beta\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M2e^{\beta J/4} \left(\cosh \beta\phi^M + \frac{J}{2\phi^M}\sinh \beta\phi^M\right)\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}\left( m + 1/2 \right)\\
	Z^{\prime\prime}(h \to 0) &=\beta^2\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[2 e^{\beta J/4}\left(\left(1 + \frac{J^2}{4{\phi^M}^2}\right)  \cosh \beta\phi^M + \frac{J}{\phi^M}\left( 1 - \frac{J}{4\beta{\phi^M}^2}\right)\sinh \beta\phi^M\right)\Sigma_M + 2\beta^2 M^2 e^{-\beta JM/2}\right]\\
\end{aligned}\end{equation}

If we take the limit of \(\beta \to \infty\), the term with \(\beta\) in \(Z^{\prime\prime}\) drops out because the terms with \(\beta^2\) will grow faster. All negative exponentials will also drop out. We get
\begin{equation}\begin{aligned}
	     Z(\beta\to\infty, h \to 0) &=e^{\beta J/4}\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}e^{\beta\phi^M_m}\right]\\
	Z^\prime(\beta\to\infty, h \to 0) &=e^{\beta J/4}\beta\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1}e^{\beta\phi^M_m}\left(- (m+1/2) + \frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h}\right)\right]\\
	Z^{\prime\prime}(\beta\to\infty, h \to 0) &=e^{\beta J/4}\beta^2\sum_{M=M_\text{min}}^{M_\text{max}}r^K_M\left[\sum_{m=-M, \atop{m\in \mathbb{Z}}}^{M-1} e^{\beta\phi^M_m}\left( (m+1/2)^2  - 2 (m+1/2)\frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h} + \left(\frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h}\right)^2\right)\right]\\
\end{aligned}\end{equation}
 In the entire nested summation, only the largest exponential out of all the \(e^{\beta\phi^M_m}\) will survive. In the limit of \(h \to 0\), we have \(\phi_m^M(h\to 0) = \frac{J}{2}(M + 1/2)\), which means that the largest exponential \(\Phi_K\) occurs for \(M = M_\text{max}\). 
\begin{equation}\begin{aligned}
	     Z(\beta\to\infty, h \to 0) &=e^{\beta J/4} e^{\beta \Phi_K}r^K_{M_\text{max}}(2M_\text{max}+1)\\
	Z^\prime(\beta\to\infty, h \to 0) &=e^{\beta J/4}\beta e^{\beta \Phi_K}r^K_{M_\text{max}}\sum_{m=-M_\text{max}, \atop{m\in \mathbb{Z}}}^{M_\text{max}-1}\left(- (m+1/2) + \frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h}\right) \\
	Z^{\prime\prime}(\beta\to\infty, h \to 0) &=e^{\beta J/4}\beta^2 e^{\beta\Phi_K}r^K_{M_\text{max}}\sum_{m=-M_\text{max}, \atop{m\in \mathbb{Z}}}^{M_\text{max}-1} \left( (m+1/2)^2  - 2 (m+1/2)\frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h} + \left(\frac{\:\mathrm{d}\phi^M_m}{\:\mathrm{d}h}\right)^2\right)\\
\end{aligned}\end{equation}
Substituting this gives
\begin{equation}\begin{aligned}
	     Z(\beta\to\infty, h \to 0) &=e^{\beta J/4} e^{\beta \Phi_K}r^K_{M_\text{max}}2M_\text{max}\\
	Z^\prime(\beta\to\infty, h \to 0) & -e^{\beta J/4}\beta e^{\beta \Phi_K}r^K_{M_\text{max}}\left(1 + \frac{J}{2\Phi_K}\right) \sum_{m=-M_\text{max}, \atop{m\in \mathbb{Z}}}^{M_\text{max}-1}\left( m + 1/2 \right) = 0\\
Z^{\prime\prime}(\beta\to\infty, h \to 0) &=e^{\beta J/4}\beta^2 e^{\beta\Phi_K}r^K_{M_\text{max}}\left(1 + \frac{J}{2\Phi_K}\right)^2 \sum_{m=-M_\text{max}, \atop{m\in \mathbb{Z}}}^{M_\text{max}-1} \left(m+1/2\right)^2\\
\end{aligned}\end{equation}

\begin{equation}\begin{aligned}
	\chi(T \to 0) = \frac{\beta\left(K+3\right)^2\Sigma_\text{max}}{K(K+1)^2} 
\end{aligned}\end{equation}
