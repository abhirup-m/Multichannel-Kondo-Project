...
Methods like CFT, Bethe ansatz and NRG \cite{affleck_1991_overscreen,Tsvelick_Weigmann_mchannel_1984,Gan_mchannel_1994,pang_cox_1991} have shown that neither the weak coupling nor the strong coupling Hamiltonians are stable fixed points of the multi-channel Kondo models. Instead, the RG flows converge to an intermediate coupling fixed point. The RG flows were found to preserve the degeneracy of the ground state \cite{pang_cox_1991,kroha_kolf_2007,zitko_fabrizio_2017}. For example, the two-channel and the three-channel Kondo models are found to be have ground state degeneracies of 2 and 3 respectively, throughout the RG flow. It was also revealed that the two and three-channel models enjoy a weak-strong duality: the model at strong coupling can also be thought of as a Kondo model but at weak coupling, possibly with a modified central complex. For example, in the two channel Kondo model, the model at strong coupling can be mapped onto the same model with a coupling parameter, but with the central Kondo spin-half impurity replaced by a new, composite spin-half object formed by the doubly-degenerate ground state of the strong coupling model \cite{kroha_kolf_2007}. The case of the three-channel model is different: there, the central impurity gets replaced by a new spin-1 impurity formed by the triply-degenerate ground state. In the former case, the strong and weak coupling models are formally identical and the model is self-dual. We will now show that these qualitative features can be captured very simply by the zero mode Hamiltonian.
