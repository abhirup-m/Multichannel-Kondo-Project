...
One final operator that we will look at is the spin parity operator \(\pi^a = S_d^a \otimes_{i=1}^K s_i^a\).
\begin{equation}\begin{aligned}
	\left[H, \pi^a\right] = J\sum_{b,j} \left[S_d^b s_j^b, S_d^a \otimes_{i=1}^K s_i^a\right] = \sum_{bj}\left(S_d^b S_d^a s^b_j s^a_j - S_d^a S_d^b s^a_j s^b_j\right) \otimes_{i<j}s_i^a \otimes_{i>j}s^a_i = 0
\end{aligned}\end{equation}
