...
\end{align}
This Hamiltonian is shown schematically in fig.~\ref{stargraph}.
...
\section{Symmetries of the zero mode Hamiltonian}
This zero mode Hamiltonian commutes with a number of operators, revealing various symmetries of the problem. The first of such operators is \(s_\text{tot}^2\). Henceforth we will drop the label \(\text{zero mode}\). To see the commuation, note that \(s_\text{tot}^2\) can be written as \(s_\text{tot}^2 = \sum_{i=x,y,z}\left(s_\text{tot}^i\right)^2\). This then gives
\begin{align}
