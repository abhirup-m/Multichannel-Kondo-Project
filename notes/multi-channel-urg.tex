...
The eigenvalues of the terminal states are \(\pm\epsilon^h_{\pm M}\). For \(h = 0\), the ground state subspace is \(K-\)fold degenerate and is formed by the negative solutions of eq.~\ref{eigenvalue}. This common \(K-\)fold degenerate eigenvalue is \(-J(M+1)/2\). The partition function is
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\chi(T \to 0) = \frac{2\beta\left(1 + \frac{J}{4\Phi}\right)^2\Sigma_M}{4M} = \frac{\beta \Sigma_\text{max}(K+2)^2}{K(K+1)^2}
\end{aligned}\end{equation}
