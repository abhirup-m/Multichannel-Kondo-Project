...
The Hamiltonian is the same as eq.~\ref{1KondoURG}, but the impurity is now a spin-\(S\) object, such that \(S_d^z \in \left[-S, -S+1,\ldots,S-1,S\right] \). We will also choose different couplings \(J_m\) for each of the eigenvalues of \(S_d^z\):
\begin{equation}\begin{aligned}
	\mathcal{H} = \sum_{k\sigma}\epsilon_{k}\tau_{k\sigma} + \sum_{k}\sum_{m=-S}^S J^z_m S_d^z s^z_{k}\ket{m}\bra{m} + \frac{1}{2}\sum_{k}\sum_{m=-S}^S J^t_m \left(S_d^+ s^-_{k} \ket{m}\bra{m} + \text{h.c.}\right)
\end{aligned}\end{equation}
The states \(\ket{m}\) act purely on the Hilbert space of the impurity spin and are defined as \(S_d^z\ket{m} = m\ket{m}\). We will use the relations:
\begin{equation}\begin{aligned}
	S_d^\pm S_d^z &= S_d^z S_d^\pm \mp S_d^\pm\\
	S_d^z S_d^\pm &= S_d^\pm S_d^z \pm S_d^\pm\\
	S_d^\pm S_d^\mp &= \vec S_d^2 - {S_d^z}^2 \pm S_d^z
\end{aligned}\end{equation}
The various terms that renormalize the Hamiltonian can be described in terms of the impurity spin operators that come into them. For example, the term that has \(S_d^z\) on both sides of the intervening Greens function can be represented as \(z-z\). There are 7 such terms: \(z|z, \pm|\mp, z|\pm, \pm|z\). Each of these terms occur both in the particle and the hole sectors.
\subsection{Particle sector}
We will now calculate each of these terms for the particle sector. The particle sector consists of those processes that start from an occupied state \(\hat n_{q\beta} = 1\).
\paragraph{\(z|z: \beta=\uparrow\)}
\begin{equation}\begin{aligned}
	\sum_{kk^\prime,m}\frac{1}{4}{J^z_m}^2 c^\dagger_{q \uparrow} c_{k^\prime \uparrow} S_d^z \ket{m}\bra{m} \frac{1}{\omega_{m, \uparrow} - D/2 + \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^z c^\dagger_{k \uparrow}c_{q \uparrow} = -\frac{1}{4}n_{q \uparrow}\sum_{kk^\prime,m}\frac{{J^z_m}^2{S_d^z}^2 c^\dagger_{k \uparrow}c_{k^\prime\uparrow} \ket{m}\bra{m}}{\omega_{m, \uparrow} - D/2 + \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(z|z: \beta=\downarrow\)}
\begin{equation}\begin{aligned}
	\sum_{kk^\prime,m}\frac{1}{4}{J^z_m}^2 c^\dagger_{q \downarrow} c_{k^\prime \downarrow} S_d^z \ket{m}\bra{m} \frac{1}{\omega_{m, \downarrow} - D/2 - \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^z c^\dagger_{k \downarrow}c_{q \downarrow} = -\frac{1}{4}n_{q \downarrow}\sum_{kk^\prime,m}\frac{{J^z_m}^2{S_d^z}^2 c^\dagger_{k \downarrow}c_{k^\prime\downarrow} \ket{m}\bra{m}}{\omega_{m, \downarrow} - D/2 - \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(+|-\)}
\begin{equation}\begin{aligned}
	\sum_{kk^\prime,m}\frac{1}{4}{J^t_m}^2 c^\dagger_{q \downarrow} c_{k^\prime \uparrow} S_d^+ \ket{m}\bra{m} \frac{1}{\omega_{m, \downarrow} - D/2 - \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^- c^\dagger_{k \uparrow}c_{q \downarrow} = -\frac{1}{4}n_{q \downarrow}\sum_{kk^\prime,m}\frac{{J^t_m}^2S_d^+ \ket{m}\bra{m} S_d^- c^\dagger_{k \uparrow}c_{k^\prime\uparrow} }{\omega_{m, \downarrow} - D/2 - \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(-|+\)}
\begin{equation}\begin{aligned}
	\sum_{kk^\prime,m}\frac{1}{4}{J^t_m}^2 c^\dagger_{q \uparrow} c_{k^\prime \downarrow} S_d^- \ket{m}\bra{m} \frac{1}{\omega_{m, \uparrow} - D/2 + \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^+ c^\dagger_{k \downarrow}c_{q \uparrow} = -\frac{1}{4}n_{q \uparrow}\sum_{kk^\prime,m}\frac{{J^t_m}^2S_d^- \ket{m}\bra{m} S_d^+ c^\dagger_{k \downarrow}c_{k^\prime\downarrow} }{\omega_{m, \uparrow} - D/2 + \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(z|+\)}
\begin{equation}\begin{aligned}
	\sum_{kk^\prime,m}\frac{1}{4}{J^z_m} J^t_m c^\dagger_{q \uparrow} c_{k^\prime \uparrow} S_d^z \ket{m}\bra{m} \frac{1}{\omega_{m, \uparrow} - D/2 + \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^+ c^\dagger_{k \downarrow}c_{q \uparrow} = -\frac{1}{4}n_{q \uparrow}\sum_{kk^\prime,m}\frac{{J^z_m} J^t_m \ket{m}\bra{m} S_d^z S_d^+ c^\dagger_{k \downarrow}c_{k^\prime\uparrow}}{\omega_{m, \uparrow} - D/2 + \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(-|z\)}
This is equal to the h.c. of the renormalization coming from \(z|+\), because \(-|z = \left(z|+\right)^\dagger \).

\paragraph{\(z|-\)}
\begin{equation}\begin{aligned}
	-\sum_{kk^\prime,m}\frac{1}{4}{J^t_m} J^z_m c^\dagger_{q \downarrow} c_{k^\prime \downarrow} S_d^z \ket{m}\bra{m} \frac{1}{\omega_{m, \downarrow} - D/2 - \frac{J}{2}S_d^z} \ket{m}\bra{m} S_d^- c^\dagger_{k \uparrow}c_{q \downarrow} = \frac{1}{4}n_{q \downarrow}\sum_{kk^\prime,m}\frac{{J^t_m} J^z_m \ket{m}\bra{m} S_d^z S_d^- c^\dagger_{k \uparrow}c_{k^\prime\downarrow}}{\omega_{m, \downarrow} - D/2 - \frac{Jm}{2}}
\end{aligned}\end{equation}

\paragraph{\(+|z\)}
This is equal to the h.c. of the renormalization coming from \(z|-\), because \(+|z = \left(z|-\right)^\dagger \).

The first four terms generate and renormalize a new three-particle term in the Hamiltonian, of the form \(J^{zz}{S_d^z}^2\sum_{kk^\prime\sigma}c^\dagger_{k\sigma}c_{k^\prime\sigma}\). The third and fourth terms also renormalize the \(J^z_m\) coupling. The terms e through h renormalize \(J^t_m\). To compare with the spin-\(\frac{1}{2}\) Kondo URG, we redefine the \(\omega_{m\sigma} \to \omega + \frac{J}{2}\left(-\sigma m + \frac{1}{2}\right) \). We also use \(\sum_\sigma \hat n_{q\sigma}= n(D)\). The total renormalization from the first four terms is
\begin{equation}\begin{aligned}
	-\frac{1}{4}\sum_{kk^\prime\sigma}\frac{n(D)\left({J^z}^2 - {J^t}^2\right) {S_d^z}^2 c^\dagger_{k \sigma}c_{k^\prime\sigma}}{\omega - D/2 + \frac{J}{4}} - \frac{1}{2}\sum_{kk^\prime}\frac{n(D){J_m^t}^2 {S_d^z} s^z_{kk^\prime}}{\omega - D/2 + \frac{J}{4}}
\end{aligned}\end{equation}
The renormalization from the last four terms is
\begin{equation}\begin{aligned}
	-\frac{1}{2}\sum_{kk^\prime, m}\frac{n(D)J^z_m J^t_m \left(S_d^+ s^-_{kk^\prime} + \text{h.c.}\right) m\ket{m}\bra{m}}{\omega - D/2 + \frac{J}{4}}
\end{aligned}\end{equation}
