...
\end{aligned}\end{equation}
Note that we are using the index \(m\) to indicate the subspace from which these eigenstates arise; \(m\) is not a quantum number for that eigenstate. The eigenvalue of \(\pi^z\) is, however, a quantum number. We have the eigenvalue relation \(\pi^z \ket{S_d^z} = (-1)^{\frac{1}{2}\left(\sigma^z_d + 2m - 1\right)}\). Applying \(\pi^z\) on these eigenstates gives
\begin{equation}\begin{aligned}
	\pi^z\ket{\Psi^M_{m,-}} = -\left(-1\right)^{\frac{1 + 2m - 1}{2}}\sqrt{x_m^M}\ket{\uparrow, M, m} + \left( -1 \right)^{\frac{-1 + 2m + 2 - 1}{2}}\left(M + m + 1\right)\ket{\downarrow,M,m+1} = \left( -1 \right) ^{m}\ket{\Psi^M_{m,-}}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\pi^x \ket{(-1)^m} = \ket{(-1)^{-1-m}} = \ket{-(-1)^m}
\end{aligned}\end{equation}
