...
\begin{align}
	\ket{g^n_m} = \frac{1}{\sqrt 2}\left[\ket{\Psi^M_{m,-}} + \ket{\Psi^M_{m+n,-1}}\right], m \in \left[-M, M-2\right] , n \in \left[1, M - m - 1\right] 
\end{align}
...
\begin{align}
	\left<\sigma^x_d\right>_m^n =\frac{1}{2}\delta_{n,1}\braket{g^1_m | \sigma_d^x | g^1_m} = \frac{1}{2}\delta_{n,1}\left[\braket{\Psi^M_{m+1,-}|\sigma_d^x| \Psi^M_{m,-}} + \text{h.c.}\right] = -\frac{1}{2}\delta_{n,1}\left[\braket{\Psi^M_{m+1,-}|\sigma_d^x \pi^x | \Psi^M_{-m-1,-}} + \text{h.c.}\right]
\end{align}
...
\begin{equation}\begin{aligned}
	\left<\sigma^x_d\right>_m^1 = -\frac{1}{2}\left[\braket{\Psi^M_{m+1,-}|\sigma_d^x \pi^x | \Psi^M_{-m-1,-}} + \text{h.c.}\right] = -\frac{\sqrt{K^2 - \left(2m+2\right)^2}}{2\left(1+K\right)}\label{mag_x}
\end{aligned}\end{equation}

For \(m = -M\), we get
\begin{gather}
	\braket{\Psi^M_{-M+1,-}|\sigma_d^x \pi^x | \Psi^M_{M - 1,-}} = \frac{\sqrt{2M-1}}{2M + 1}\\
	\left<\sigma^x_d\right>_{-M}^1 = -\frac{1}{2}\left[\braket{\Psi^M_{-M+1,-}|\sigma_d^x \pi^x | \Psi^M_{M - 1,-}} + \text{h.c.}\right] = -\frac{\sqrt{2M-1}}{2M + 1} = -\frac{\sqrt{K-1}}{K+1}
\end{gather}

We can similarly calculate the magnetization along the \(z-\)direction in the state \(\ket{g^1_m}\). The expectation value of \(\sigma^d_z\) will be non-zero only in the diagonal terms: \(\braket{\Psi^M_m | \sigma^z_d | \Psi^M_{m^\prime}} = \delta_{mm^\prime}\braket{\Psi^M_m | \sigma^z_d | \Psi^M_{m}}\). Using this, we get
\begin{equation}\begin{aligned}
...
Combining this with eq.~\ref{mag_x}, we get
\begin{equation}\begin{aligned}
	\left(\left<\sigma^x_d\right>_m^1\right)^2 + \left(\frac{1}{2}\left<\sigma^z_d\right>_m^1\right)^2 = \left[\frac{K}{2\left( 1+K \right) }\right] ^2
\end{aligned}\end{equation}
This relation constrains the magnetization along \(z\) in terms of the magnetization along \(x\) (which is related to the t'hooft loop \(\pi^x\)) and a function of \(K\) (the function is quantized because \(K\) can only take integer values).
